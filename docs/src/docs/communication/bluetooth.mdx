...
Bluetooth on the device operates in two modes in accordance to the BLE standard, GAP and GATT modes. The GAP mode advertises the device such that it can be paired with. Once the device is paired with a client, the GATT mode exposes several characteristic attributes that contains device and sensor data. Currently, the IP address and MAC address of the device can be retrieved from the device. Additional characteristic attributes can be read that contains sensor data if the device was built with the device firmware (currently supports DHT and BME680 sensors).
